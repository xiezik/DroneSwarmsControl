...
from fq.msg import Header, ActorControlInfos, ActorHitInfos ,ActorReleaseChaffInfos, ActorChangeDirectionInfos, ActorAirplanes, ActorBuildings, ActorDroneSwarms, ActorVehicles, ActorWarships, ActorZones, ActorHumans, ActorEquipments
...
class FqTopic(Enum):
    ...
    # 蜂群控制返回值
    drone_swarm_control_info = "fq/drone_swarm_control_info"

    # 蜂群打击返回值
    drone_swarm_hit_info = "fq/drone_swarm_hit_info"

    # 蜂群改变干扰方向返回值
    drone_swarm_change_direction_info = "fq/drone_swarm_change_direction_info"

    # 蜂群释放箔条返回值
    drone_swarm_release_chaff_info = "fq/drone_swarm_release_chaff_info"
    ...
    # 人类参数
    human_param = "fq/human"

    # 特殊区域参数
    zone_param = "fq/zone"

    # 特殊装备参数
    equipment_param = "fq/equipment"

FqTopic2Message = {
    FqTopic.drone_swarm_param: ActorDroneSwarms,
    FqTopic.drone_swarm_control_info: ActorControlInfos,
    FqTopic.drone_swarm_change_direction_info: ActorChangeDirectionInfos,
    FqTopic.drone_swarm_hit_info: ActorHitInfos,
    FqTopic.drone_swarm_release_chaff_info: ActorReleaseChaffInfos,
    ...
    FqTopic.human_param: ActorHumans,
    FqTopic.equipment_param: ActorEquipments
} 
...
class SimMessageNode(Node):
    ...
    def __init__(self):
        ...
        self._subscription_handler = {
            ...
            FqTopic.human_param: SimMessageNode.TopicSubscriptionHandler(self, FqTopic.human_param),
            FqTopic.equipment_param: SimMessageNode.TopicSubscriptionHandler(self, FqTopic.equipment_param),
        }
        self._control_command_publisher = self.create_publisher(
            FqTopic2Message.get(FqTopic.drone_swarm_control_info),
            FqTopic.drone_swarm_control_info.value,
            10
        )
        ...
        self._change_direction_publisher = self.create_publisher(
            FqTopic2Message.get(FqTopic.drone_swarm_change_direction_info),
            FqTopic.drone_swarm_change_direction_info.value,
            10
        )

        self._release_chaff_publisher = self.create_publisher(
            FqTopic2Message.get(FqTopic.drone_swarm_release_chaff_info),
            FqTopic.drone_swarm_release_chaff_info.value,
            10
        )
    ...
    def publish_change_direction_info(self, msg):
        self._change_direction_publisher.publish(msg)
        self.get_logger().info(f'send change direction info: {msg}')
        
    def publish_release_chaff_info(self, msg):
        self._release_chaff_publisher.publish(msg)
        self.get_logger().info(f'send release chaff info: {msg}')
...
class API(object):
    ...
    @staticmethod
    def publish_change_direction_info(msg):
        API._sim_node.publish_change_direction_info(msg)
        
    @staticmethod
    def publish_release_chaff_info(msg):
        API._sim_node.publish_release_chaff_info(msg)
    ...
    @staticmethod
    def next_human_param():
        return API.next_data(FqTopic.human_param)
    
    @staticmethod
    def next_equipment_param():
        return API.next_data(FqTopic.equipment_param)
